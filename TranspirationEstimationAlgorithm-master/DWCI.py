...
def DWCIcalcSimple(Rg_pot,ET,GPP,VPD,NEE,ET_sd,GPP_sd):
    ...
    for rep in range(repeats):
        ...
        GPP_DayCycle=(Rg_pot.reshape(-1,48)*(GPP.reshape(-1,48).mean(axis=1)/Rg_pot.reshape(-1,48).mean(axis=1))[:,None]+GPP_noise.reshape(-1,48)).reshape(-1)
        ET_DayCycle=(Rg_pot.reshape(-1,48)*(ET.reshape(-1,48).mean(axis=1)/Rg_pot.reshape(-1,48).mean(axis=1))[:,None]+ET_noise.reshape(-1,48)).reshape(-1)
        StN[rep]=daily_corr(ET_DayCycle,GPP_DayCycle,Rg_pot)
